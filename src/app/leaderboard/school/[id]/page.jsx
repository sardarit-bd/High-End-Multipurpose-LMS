...
import { FaArrowLeft, FaUniversity, FaUsers, FaTrophy, FaAward } from 'react-icons/fa';
...
const SchoolLeaderboardPage = () => {
  ...
  const { data: schoolData } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get(`/gamification/leaderboard/schools?schoolId=${schoolId}`);
  ...
  const { data: studentsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get(`/gamification/leaderboard?schoolId=${schoolId}&limit=50`);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#ECFDF5]">
        <Head>
          <title>{schoolDetails?.schoolName || 'School'} Leaderboard - SDG Oasis</title>
        </Head>

        {/* Header */}
        <div className="bg-gradient-to-r from-[#059669] to-[#0f9293] text-white">
            ...
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold">
                  {schoolDetails?.schoolName || 'School'} Leaderboard
                </h1>
                <p className="text-white/80">Student rankings within this institution</p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="text-2xl font-bold">{schoolDetails.studentCount}</div>
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
                  <div className="flex items-center space-x-3">
                    <FaTrophy className="w-8 h-8 text-white/80" />
                    <div>
                      <div className="text-2xl font-bold">{formatPoints(schoolDetails.totalPoints)}</div>
                ...
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="text-2xl font-bold">{formatPoints(schoolDetails.averagePoints)}</div>
          ...
          <div className="bg-white rounded-[1rem] shadow-[0_4px_12px_rgba(0,0,0,0.1)] p-8 mb-8">
            ...
            {studentData.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-[#064E3B] opacity-80">No student data available for this school.</p>
              </div>
            ) : (
              <LeaderboardTable
