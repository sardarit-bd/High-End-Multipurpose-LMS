...
export default function EditPackagePage() {
  ...
  // Fetch package data and courses
  useEffect(() => {
    fetchPackageData();
  ...
  const fetchPackageData = async () => {
    try {
      ...
      setFormData({
        ...
        accessDays: packageData.accessDays?.toString() || "365",
        features: packageData.features?.length > 0 ? packageData.features : [""],
        isActive: packageData.isActive || true
      });
      
      // Set selected courses
      if (packageData.courseIds && packageData.courseIds.length > 0) {
        setSelectedCourses(packageData.courseIds.map(course => ({
  ...
  // Check if a course is already in the package
  const isCourseInPackage = (courseId) => {
    return selectedCourses.some(course => course._id === courseId);
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[var(--color-text)]/70 text-sm md:text-base mt-1">
                  Update package details and courses
                </p>
              </div>
            </div>
          </div>
                ...
                {coursesLoading ? (
                  ...
                  <div className="space-y-2">
                    {filteredCourses.map((course) => {
                      const isSelected = isCourseInPackage(course._id);
                      return (
                        <div
                          key={course._id}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <button
                    type="button"
                    onClick={() => router.push("/dashboard/admin/packages")}
                    className="w-full border border-gray-300 text-gray-700 py-3 px-6 rounded-xl font-medium hover:bg-gray-50 transition"
                  >
                    Cancel
                  </button>
                </div>
